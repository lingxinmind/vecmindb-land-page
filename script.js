...
const translations = {
    'zh': {
        ...
        'feat-5-h': '零外部依赖',
        'feat-5-p': '内置 BGE-M3 旗舰权重 ONNX 引擎。一个 Docker 镜像启动。不需要 Python 环境、不需要 PyTorch、不需要 OpenAI API Key、不需要任何外部 Embedding 服务。',
        ...
        'code-cluster-comment-1': '# 1. 保存左侧标签页的 docker-compose.yml 到本地，并在同目录下运行启动集群:',
        'code-cluster-comment-2': '# 2. 集群启动自动探测硬件并自动调优，检查集群拓扑健康状态',
        'code-cluster-comment-3': '# 3. 分布式多节点高并发存入记忆，数据根据分区哈希路由与重平衡',
        'code-cluster-comment-4': '# 4. 无损平滑升级集群 (Raft 数据卷自动保留):',
        ...
        'code-comment-1-1': '# 官方 GitHub GHCR 源 (生产推荐: 挂载数据卷 vecmindb-data，本地映射可用 -v ./vecmindb-data:/home/vecminDB/data):',
        'code-comment-1-2': '# 备选：国内阿里云极速专线 (免翻墙秒级下载):',
        ...
        'nav-dashboard': '控制台',
        'dash-title': '内置零依赖可视化脑图控制台',
        'dash-subtitle': '无需安装额外软件，浏览器一键访问。实时掌握智能体大脑的所有状态演化。',
        'dash-feat-1-title': 'LTSM 遗忘线与记忆衰减监测',
        'dash-feat-1-desc': '以曲线直观展现 Episodic Memory 的半衰期衰减与降噪过滤进度。',
        'dash-feat-2-title': 'PCA 知识质心自进化演化图',
        'dash-feat-2-desc': '实时观察多条关联记忆如何通过 Welford 在线均值和 DP-Federated PCA 蒸馏合并为稳定的知识质心。',
        'dash-feat-3-title': '集群强一致 Raft 拓扑图',
        'dash-feat-3-desc': '呈现分布式节点的心跳状态、主备关系及日志复制轨迹，确保 100% 数据一致性。',
        'dash-run-desc': '在控制台直接执行语义检索，秒级回溯智能体在各个主权域中的关联记忆。',
        'qs-dash-tip-title': '服务启动后即可访问可视化控制台',
        'qs-dash-tip-desc': '在浏览器打开内置大盘，若按上述生产推荐模式启动，可使用 my-secret-key 作为 Admin Key 登录，且容器更新重建时向量数据和配置完好保留。',
    ...
    'en': {
        ...
        'feat-5-p': 'Built-in BGE-M3 weight ONNX engine. One Docker image. No Python. No PyTorch. No OpenAI API key. No external embedding service. Single binary, single container.',
        ...
        'code-cluster-comment-1': '# 1. Save docker-compose.yml from the other tab locally, then run in the same directory:',
        ...
        'code-cluster-comment-4': '# 4. Seamless Cluster Upgrade (Raft volume auto-persisted):',
        ...
        'code-comment-1-1': '# GitHub GHCR (Production: mounts named volume vecmindb-data, or use -v ./vecmindb-data for local folder):',
        'code-comment-1-2': '# Alternative: Aliyun Registry (domestic mirror in China, instant download):',
        ...
        'dash-title': 'Built-in Zero-Dependency Dashboard',
        'dash-subtitle': 'No external software required. One-click access in your browser to monitor all semantic memory state evolutions.',
        'dash-feat-1-title': 'LTSM Forgetting & Decay Monitoring',
        'dash-feat-1-desc': 'Visualize episodic memory half-life decay curves and noise filtering progress dynamically.',
        'dash-feat-2-title': 'PCA Centroid Evolution Graph',
        'dash-feat-2-desc': 'Observe real-time memory compression and abstraction into stable knowledge centroids via Welford and DP-Federated PCA.',
        'dash-feat-3-title': 'Raft Cluster Topology Console',
        'dash-feat-3-desc': 'Inspect distributed node heartbeats, leader consensus states, and real-time log replication streams.',
        'dash-run-desc': 'Execute semantic vector queries directly in the browser to instantly trace associated agent memories across sovereignty domains.',
        'qs-dash-tip-title': 'Access Visual Console Once Started',
        'qs-dash-tip-desc': 'Open the built-in console. If launched using the production-ready command above, log in with my-secret-key. All vectors and settings will persist across updates.',
